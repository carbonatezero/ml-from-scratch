...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        sample_var = np.mean(x_centered * x_centered, axis=0)
...
def batchnorm_backward(dout, cache):
    ...
    dxhat = dout * gamma
    dinv_std = np.sum(dxhat * x_centered, axis=0)
    dx_centered = dxhat * inv_std
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x_centered = x - sample_mean
    sample_var = np.mean(x_centered * x_centered, axis=1, keepdims=True)
    inv_std = 1.0 / np.sqrt(sample_var + eps)
    x_hat = x_centered * inv_std
    out = gamma * x_hat + beta
    cache = (x_hat, gamma, x_centered, inv_std, sample_var, eps)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    sN, sC, sH, sW = x_pad.strides
    x_cols = np.lib.stride_tricks.as_strided(
        x_pad,
        shape=(N, C, H_out, W_out, HH, WW),
        strides=(sN, sC, stride * sH, stride * sW, sH, sW),
        writeable=False,
    )
    out = np.tensordot(x_cols, w, axes=([1, 4, 5], [1, 2, 3])).transpose(0, 3, 1, 2)
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        ...
        for j in range(W_out):
            ws = j * stride
            x_slice = x_pad[:, :, hs : hs + HH, ws : ws + WW]
            dout_slice = dout[:, :, i, j]
            dw += np.tensordot(dout_slice, x_slice, axes=([0], [0]))
            dx_pad[:, :, hs : hs + HH, ws : ws + WW] += np.tensordot(
                dout_slice, w, axes=([1], [0])
            )
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_out):
        ...
        for j in range(W_out):
            ws = j * stride
            out[:, :, i, j] = np.max(
                x[:, :, hs : hs + pool_height, ws : ws + pool_width], axis=(2, 3)
            )
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        ...
        for j in range(W_out):
            ws = j * stride
            x_slice = x[:, :, hs : hs + pool_height, ws : ws + pool_width]
            mask = x_slice == np.max(x_slice, axis=(2, 3), keepdims=True)
            dx[:, :, hs : hs + pool_height, ws : ws + pool_width] += (
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    var = np.mean((x_group - mean) ** 2, axis=(2, 3, 4), keepdims=True)
    inv_std = 1.0 / np.sqrt(var + eps)
    x_hat = ((x_group - mean) * inv_std).reshape(N, C, H, W)
    out = gamma * x_hat + beta
    cache = (G, x, x_hat, mean, var, inv_std, gamma, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    G, x, x_hat, mean, var, inv_std, gamma, eps = cache
    N, C, H, W = dout.shape
    ...
    m = (C // G) * H * W
    dvar = np.sum(dxhat * x_centered, axis=(2, 3, 4), keepdims=True) * (
        -0.5 * (var + eps) ** (-1.5)
    )
    dmean = -np.sum(dxhat * inv_std, axis=(2, 3, 4), keepdims=True) - (
        2.0 / m
    ) * dvar * np.sum(x_centered, axis=(2, 3, 4), keepdims=True)
    dx = (
        dxhat * inv_std + (2.0 / m) * x_centered * dvar + dmean / m
    ).reshape(N, C, H, W)
