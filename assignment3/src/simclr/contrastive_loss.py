...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denominator_k = sum(
            torch.exp(sim(z_k, out[j]) / tau) for j in range(2 * N) if j != k
        )
        denominator_k_N = sum(
            torch.exp(sim(z_k_N, out[j]) / tau)
            for j in range(2 * N)
            if j != k + N
        )
        total_loss += -torch.log(numerator_k / denominator_k)
        total_loss += -torch.log(numerator_k_N / denominator_k_N)
        ##############################################################################
        # TODO: Start of your code.                                                  #
        #                                                                            #
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    mask = ~torch.eye(2 * N, device=out.device, dtype=torch.bool)
    ...
    positive_similarities = sim_positive_pairs(out_left, out_right)
    numerator = torch.exp(torch.cat([positive_similarities, positive_similarities]) / tau)
